...
// Change it into session variable of user id
$user = $_POST['age'];
if ($stmt = $con->prepare('INSERT INTO guest_info(user_id, name, sex, age, rooms, address, check_in, check_out, username, email, contact, category, purpose, payment_by, additional, file_name, file_type, file_data) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')) {
	$stmt->bind_param('ssssssssssssssssss',$user, $_POST['name'], $_POST['sex'], $_POST['age'], $_POST['rooms'], $_POST['address'], $checkin_date, $checkout_date, $_POST['username'], $_POST['email'], $_POST['contact'], $_POST['category'], $_POST['purpose'], $_POST['payment'], $_POST['additional'], $name, $type, $data);
	$stmt->execute();
	echo 'You have successfully booked the rooms!';
	$_SESSION['number_of_rooms'] = $_POST['rooms'];
	$_SESSION['check_in'] = $checkin_date;
	$_SESSION['check_out'] = $checkout_date;
	$query = "SELECT MAX(booking_id) FROM guest_info";
